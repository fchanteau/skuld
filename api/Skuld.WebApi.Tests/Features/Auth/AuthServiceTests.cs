...
namespace Skuld.WebApi.Tests.Features.Auth
{
	public class AuthServiceTests
	{
		...
		public async Task AddUserAsync_Should_Return_UserCreated ()
		{
			...
			_userRepository.TryGetByIdAsync (Arg.Any<long> ()).Returns (userCreated);
		...
		[Fact]
		public async Task AddUserAsync_Should_Throw_SkuldException_UserNotFound ()
		{
			// Given
			var payload = new AddUserPayload
			{
			...
			_dateTimeProvider.UtcNow.Returns (new DateTime (2023, 6, 14));
			_tokenProvider.BuildRefreshToken (Arg.Any<User> (), Arg.Any<DateTime> ()).Returns (new RefreshToken ());
			_userRepository.TryGetByIdAsync (Arg.Any<long> ()).Returns (Task.FromResult<User?> (null!));

			// When
			var action = () => _authService.AddUserAsync (payload);
		...
		public async Task LoginAsync_Should_Return_TokenInfoResponse_With_Newly_RefreshToken ()
		{
			...
			_passwordProvider.Received (1);
			_userRepository.Received (1);
			_tokenProvider.Received (2);
			_refreshTokenRepository.Received (2);
			_dateTimeProvider.Received (1);
